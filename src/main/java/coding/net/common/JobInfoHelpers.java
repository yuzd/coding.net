...
import static coding.net.plugin.subscriber.CDEventsSubscriber.isApplicableFor;
...
public final class JobInfoHelpers {
    ...
    public static Predicate<Job> isAlive() {
        return new Predicate<Job>() {
            @Override
            public boolean apply(Job job) {
                return !from(CDEventsSubscriber.all()).filter(isApplicableFor(job)).toList().isEmpty();
            }
        };
    }
